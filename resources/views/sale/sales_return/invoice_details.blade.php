                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                @foreach($order->logs as $log)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $log->quantity }}</td>
                                        @if(auth()->user()->role == 2)
                                            <td>৳{{ number_format($log->unit_price + nbrSellCalculation($log->unit_price), 2) }}</td>
                                            <td>৳{{ number_format(($log->unit_price + nbrSellCalculation($log->unit_price)) * $log->quantity, 2) }}</td>
                                        @else
                                            <td>৳{{ number_format($log->selling_price, 2) }}</td>
                                            <td>৳{{ number_format($log->sale_total, 2) }}</td>
                                        @endif
